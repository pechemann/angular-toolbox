...
describe('SubscriptionService', () => {
  ...
  const illegalAccessError: SubscriptionError =  new SubscriptionError("Illegal Access Error: you must call the register() method before invoking the append() method.")
  ...
  it('append() should throw an error if register() method have not be called before', () => {
    const subscription1 = observable1.subscribe();
    expect(()=> service.append(subscription1)).toThrow(illegalAccessError);
  });
  ...
  it('clearAll() should destroy reference to the last register() invokation for the specified reference', () => {
    ...
    service.register(ref1, subscription1);
    service.clearAll(ref1);
    expect(()=> service.append(subscription2)).toThrow(illegalAccessError);
  });
