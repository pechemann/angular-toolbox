...
import { AtxLogLevel, EMPTY_STRING, OBJECT, STRING } from "../../../util";
...
export class AtxLogUtil {
     ...
     */
    public static logListToString(logList: AtxLog[]): string {
        const len: number = logList.length - 1;
        let logs: string = EMPTY_STRING,
            i: number = 0;
        for(; i <= len; ++i) {
            const log: AtxLog = logList[i];
            const errorTxt: string = log.level === AtxLogLevel.LOG ? EMPTY_STRING : ERROR_STRING;
            logs += `[${AtxLogUtil.dateToHHMMSS(log.timestamp)}]${errorTxt}[${log.caller}][${log.message}][${AtxLogUtil.metadataToString(log.metadata)}]\n`;
        }
        return logs;
    }
    ...
    public static metadataToString(obj: any): string {
        ...
        if (Array.isArray(obj)) return AtxLogUtil.formatArray(obj);
        ...
        for(; i <= len; ++i) {
            ...
            else if (Array.isArray(value)) result += `${key}=${AtxLogUtil.formatArray(value)}${getStringEnd(i, len)}`;
            else if (typeof value === OBJECT) result += `${key}=${AtxLogUtil.metadataToString(value)}${getStringEnd(i, len)}`;
    ...
    private static formatArray(array: any[]): string {
        ...
        for(; i <= len; ++i) {
            ...
            else if (Array.isArray(value)) result += `${AtxLogUtil.formatArray(value)}${getStringEnd(i, len)}`;
            else if (typeof value === OBJECT) result += `${AtxLogUtil.metadataToString(value)}${getStringEnd(i, len)}`;
